    ...
    renderEditableData(data, keyName, ref, depth = 0) {
        ...
        const fillObjectChildren = (key, value) => {
            ...
            children.push(
                ...
                >
                    ...
                    >{key}</span>
                        {typeVal &&
                            <span className={c('editable-type', value != null && value.constructor.name)}>
                                {typeVal}
                                {!isRecu && <span className={c('editable-operation editable-operation-add')}
                                      onClick={evt => {
                                          let parent = parentUtil(evt.target, 'editable-value')
                                          let lastChildId = getLastChildId(parent)

                                          if (this.state.lastChildId === lastChildId && this.state.depth === depth + 1) {
                                              this.unfold(toggleId)
                                              return
                                          }
                                          this.unfold(toggleId)
                                          this.setState({lastChildId, depth: depth + 1})
                                      }}
                                >+</span>
                            }
                            </span>
                    ...
                    {!isRecu
                        ? renderEditableData(value, key, ref[keyName], depth + 1)
                        : (
        ...
        if (data instanceof Array) {
            ...
            for (var key in data) {
                var value = data[key]
                index++
                fillObjectChildren(key, value)
            }
        }
        if (Math.abs(this.id - lastChildId) === 0
            && stateDepth - depth === 0) {
            children.push(
                <li
                ...
                >
                    <label className="editable-key-type">
                        <span className="editable-key">
                            <input type="text"
                                   ...
                                   onBlur={evt  => {
                                       setTimeout(() => {
                                           if (this.addInputVal !== document.activeElement) {
                                               this.addInputControlBlur(ref)
                                           }
                                       }, 0)
                                   }}
                    ...
                    {renderEditableControlFrame(
                        ...
                        <input type="text"
                               ...
                               onBlur={evt => {
                                   setTimeout(() => {
                                       if (this.addInputKey !== document.activeElement) {
            ...
            <div className={`attr-editable-main attr-editable-name-${opt.attrTitle.toLowerCase()}`}>
                ...
                {addAble &&
                    <span
                        ...
                        onClick={evt => {
                            let parent = parentUtil(evt.target, 'attr-editable-main')
                            parent = findChild(parent, 'editable-object', true)
                            let root = findChild(parent, 'editable-value', true)
                            let lastChildId = getLastChildId(root)
                            if (this.state.lastChildId === lastChildId && this.state.depth === 0) {
                                this.setState({
                                    unfolded: true
                                })
                                return
                ...
                {this.renderEditableData(this.ref[opt.attrName], opt.attrName, this.ref, 0)}
            </div>
        )
    }
